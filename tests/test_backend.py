...
def test_dask_mean_var():
    ...
    assert np.allclose(np.mean(x_np, axis=0), fau_mean_dask)
    assert np.allclose(np.std(x_np, axis=0), fau_var_dask**0.5)
...
@pytest.mark.multimodal
def test_dask_min_max():
    import dask.array as da

    x_dask = da.from_array(x_np)

    assert np.allclose(np.min(x_np, axis=0), stats.min(x_dask, axis=0))
    assert np.allclose(np.max(x_np, axis=0), stats.max(x_dask, axis=0))
...
@pytest.mark.multimodal
def test_spatially_variable_genes_dask():
    import dask.array as da
