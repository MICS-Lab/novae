...
def mean_pide_score(adata: AnnData | list[AnnData], slide_key: str, obs_key: str) -> float:
    """Mean PIDE over all slides"""
    return np.mean(
...
def pide_score(adata: AnnData, obs_key: str) -> float:
    """Percentage of inter-domain edges (PIDE)"""
...
def _iter_uid(adatas: AnnData | list[AnnData], slide_key: str, obs_key: str | None = None):
    if isinstance(adatas, AnnData):
        adatas = [adatas]

    if obs_key is not None:
        categories = set.union([set(adata.obs[obs_key].unique()) for adata in adatas])
        for adata in adatas:
            adata.obs[obs_key] = (
